...
const QUESTION_LEAD_IN_MS = 1800;
...
function buildQuestions(songs, requestedCount) {
  const safeCount = Math.max(
    MIN_QUESTION_COUNT,
    Math.min(MAX_QUESTION_COUNT, Number(requestedCount) || DEFAULT_QUESTION_COUNT)
  );
...
async function settleRoom(room, now) {
  ...
  if (room.revealUntilMs) {
    ...
    const questionEndsAtMs = questionStartedAtMs + QUESTION_DURATION_MS;
    await db.collection(ROOMS_COLLECTION).doc(room._id).update({
      data: {
  ...
  if (allPlayersAnswered(room, question.id) || now >= room.questionEndsAtMs) {
...
function publicRoom(room, openId, now) {
  ...
  return {
    ...
    canStart: room.status === "waiting" && players.length === 2,
    myAnswer: myAnswer
      ? {
...
async function startRoom(event, openId, now) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if ((room.players || []).length !== 2) throw new Error("等好友加入后再开始");
    const questionStartedAtMs = now + QUESTION_LEAD_IN_MS;
    await ref.update({
      data: {
        status: "playing",
        currentIndex: 0,
        questionStartedAtMs,
        questionEndsAtMs: questionStartedAtMs + QUESTION_DURATION_MS,
        revealUntilMs: 0,
...
async function answerRoom(event, openId, now) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (now < room.questionStartedAtMs) throw new Error("还没开始播放");
    ...
    const everyoneAnswered = players.length === 2
      && players.every((player) => playerAnswerForQuestion(player, questionId));
    await ref.update({
      data: {
        players,
        revealUntilMs: everyoneAnswered ? now + REVEAL_DURATION_MS : 0,
        updatedAtMs: now
      }
    });
  });
...
exports.main = async (event = {}) => {
  ...
  try {
    ...
    return {
      ok: false,
      message: (error && error.message) || "前奏挑战暂时不可用"
    };
  }
};
