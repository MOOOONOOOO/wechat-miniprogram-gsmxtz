...
- 决选结果支持“晋级路线｜表格结果”两种海报样式，均完整展示 16/32 首歌曲从首轮到冠军的决选结果，并以现有墨绿主题色高亮冠军路径；页面会记住用户选择，保存图片跟随当前样式。聊天分享卡片继续使用冠军缩略图，并把好友带到正式的“决战歌曲之巅”创建流程；本地比赛数据仍不公开。
...
- 圣诞树结果保存视频使用小程序原生 `wx.createMediaRecorder`：本地母版通过离屏 WebGL 逐帧录制，无需上传或等待云函数，等待页显示真实录制帧进度；开发者工具不支持该录制 API，需要使用基础库 2.16.1 及以上的真机测试。
